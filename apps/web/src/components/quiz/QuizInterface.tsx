...
export function QuizInterface({ sourceId }: QuizInterfaceProps) {
    ...
    useEffect(() => {
        const loadExistingQuiz = async () => {
            try {
                const response = await fetch(`http://localhost:8001/ai/quiz/${sourceId}`);
                if (response.ok) {
    ...
    if (questions.length === 0) {
        return (
            <div className="text-center py-16 bg-gray-50 dark:bg-slate-700 rounded-xl border border-dashed border-gray-200 dark:border-slate-700 transition-colors duration-300">
                ...
                <p className="text-gray-500 dark:text-slate-400 mb-6">Generate a quiz based on this content to reinforce your learning.</p>
                <button
    ...
    if (quizCompleted) {
        return (
            <div className="text-center py-12">
                ...
                <button
                    onClick={generateQuiz}
                    className="flex items-center mx-auto px-6 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 font-medium transition-colors duration-300"
                >
